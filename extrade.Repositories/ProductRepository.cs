...
namespace UmbrellaTrading.Repositories
{
    ...
    {
        public ProductRepository(UmbrellaTradingContext context) : base(context)
        { }
                        ...
                        )
        {
            ...
            var Result =
                query
                .Where(p => p.Status == Models.ProductStatus.accepted
        ...
        public ProductViewModel ProductStatus(int ID)
        {
            ...
            filter = filter.Or(p => p.ID == ID);
            var query = GetbyID(filter);
            if (query.Status == Models.ProductStatus.pending)
            {
                query.Status = Models.ProductStatus.accepted;
            }
            else if (query.Status == Models.ProductStatus.rejected)
                query.Status = Models.ProductStatus.accepted;

            else if (query.Status == Models.ProductStatus.accepted)
                query.Status = Models.ProductStatus.rejected;
            return base.Update(query).Entity.ToViewModel();
        }
