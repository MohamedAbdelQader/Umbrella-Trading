...
using static UmbrellaTrading.ViewModels.OrderDetailsExtension;
...
namespace UmbrellaTrading.Repositories
{
    public class OrderDetailsRepositoty : GeneralRepositories<OrderDetails>
    {
        ...
        public OrderDetailsRepositoty(ProductRepository _prodRepo,
            UmbrellaTradingContext _DBContext,
        ...
        //{
        //    var ProductsID = base.GetList().Select(p => p.ProductId).Distinct().ToList();
        //    var finalresult = new List<ProductViewModel>();
        //    for (int i = 0; i < ProductsID.Count(); i++)
        //    {
        //        var Product = ProdRepo.GetProduct(ProductsID[i]);
        //        finalresult.Add(Product.ToModel().ToViewModel());
        //    }
        //    return finalresult.OrderBy(p => p.ID).Take(count).ToList();
        //}
